...
// value는 EDITORIAL 전용(±1~5 정수, 앵커 보정 한정·최대 100건), 그 외 award는 공란
...
5. **리그 계수 `LEAGUE_COEFF`** (04-ratings.ts 상단 상수) — **국내 플옵 가점에만 적용, Worlds·MSI·개인 수상 제외** (이중 페널티 방지):
   LCK/LPL: 1.0 (기준) / LEC: 0.95 / LCS: 0.85
   조정 기준: 동년 LCS 최상위 선수가 LCK 최상위와 OVR 차이 ≥ 3이 되도록 유지
